...
| ログイン不要オプション検討・実装（従業員向け）               | AI/USER |      | 完了   | 共有URL機能として実装完了。ユーザーごとの共有ID生成、ソース選択状態同期、認証不要アクセス、30日間有効期限、Edge Function経由でのRLSバイパス対応。 |
...
- [x] No.68 各機能向けAPIエンドポイント作成 (Supabase Edge Functions or other) (担当: AI, 状態: 完了, 詳細: 主要機能向けEdge Functions実装完了。共有機能用のcreate-share-config、get-share-config、shared-qa追加。)
...
### 共有機能（新規追加）
- [ ] 管理者が「共有」ボタンから共有URLを生成できる
- [ ] 生成された共有URLが一意で、30日間有効である
- [ ] 共有URLにアクセスすると認証不要で閲覧専用ページが表示される
- [ ] 共有ページで管理者が選択したソースのみがAIチャットで参照される
- [ ] 共有ページで管理者のメモ一覧が閲覧できる
- [ ] 共有ページのAIチャットが正常に動作する
- [ ] 期限切れの共有URLはアクセスできない
