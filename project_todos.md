...
| タスク名                                               | 担当者 | 期限 | 進捗状況 | 備考                                       |
| :----------------------------------------------------- | :----- | :--- | :------- | :----------------------------------------- |
| AIチャットボットバックエンド実装（自然言語処理）       | AI/USER |      | 進行中   | Google Gemini API, LangChain を使用し、マニュアルに基づく基本的な回答生成ロジックは実装済み。現在、回答精度と参照ソースのチューニングを実施中。 |
...
| AIによる次アクション提案機能（レコメンド機能）                       | AI/USER |      | 未着手   | **仕様詳細:** <br>- 更新トリガー: 「アイディアを更新」ボタン押下時のみ。 <br>- 初回表示: 「『アイディアを更新』ボタンを押して、最初の提案を取得しましょう」等の案内。 <br>- 更新中表示: ローディングスピナー等の表示。 <br>- 生成不可時: 「適切な提案が見つかりませんでした」等のメッセージ。 <br>- 最大表示数: 10件（5行2列イメージ）。 <br>- キャッシュ: localStorage等に保存しリロード後も保持。有効期限なし。 <br>- エラー処理: APIエラー時に適切なメッセージ表示。 <br>- 対象ソース: 初期は全ソース、将来的にはユーザー選択ソース。 |
