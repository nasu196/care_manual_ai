...
| タスク名                                       | 担当者    | 期限 | 進捗状況 | 備考                                                       |
| :--------------------------------------------- | :-------- | :--- | :------- | :--------------------------------------------------------- |
| 共有デバイス向けシンプルUIデザイン             | USER/AI   |      | 完了   | PC、タブレット対応。編集権限なしモード（閲覧専用モード）として実装完了。AIチャット60%・メモ管理40%の最適化されたレイアウト、重要フラグ表示強化、スクロール制御など。 |
...
| 管理者作成メモ閲覧機能実装                     | AI/USER   |      | 完了   | 編集権限なしモードでの読み取り専用メモ閲覧機能実装完了。重要フラグ表示、ソート機能（重要→日付順）、スクロール制御を含む。                                                            |
| 重要情報ワンタップアクセス機能UIデザインと実装 | AI/USER   |      | 完了   | 重要フラグ機能として実装完了。重要メモの視覚的強調表示、一覧の優先表示、パルスアニメーション等。                                                            |
| Clerkログイン時の文言最適化                    | USER/AI   |      | 未着手   | ログイン画面・サインアップ画面の日本語文言や説明、ボタンラベル等を最適化。利用者に分かりやすい表現・案内を検討。 |
...
| マニュアル管理機能（アップロード、一覧、最新版表示）実装     | AI/USER |      | 完了   | `SourceManager.tsx` にて基本的なファイル操作UI（アップロード、一覧、削除、名前変更 ※ファイル名はASCIIのみ、**ソース選択状態の永続化**）を実装。日本語ファイル名対応、最新版管理ロジックは未実装。PDF対応。**チャット時の参照ソース選択機能と連携。モバイル表示時のスタイル調整完了。** |
| ソース元データ表示機能実装                                   | AI/USER |      | 完了     | PDFのハンバーガーメニューに「ソース元データを表示」機能を追加。manual_chunksテーブルからテキストデータを取得し、大型モーダルで表示。OCR抽出部分は「[OCR抽出テキスト]」マーカーで識別可能。レスポンシブデザイン対応（最大幅4xl、高さ80vh）。 |
| 管理者作成メモ作成・編集・管理機能実装                       | AI/USER |      | 完了      | リッチテキストエディタ（Tiptap）導入、新規メモ作成UIフロー（NotebookLM風表示切替）、エディタ内の箇条書き（ul, ol）・見出し（h1-h3）スタイル調整完了。CRUD操作（作成、閲覧、更新、削除）のフロントエンドおよびSupabase Edge Functions連携完了。重要フラグ機能の追加（DB、API、UI連携完了）。UI微調整含む。 |
| AIへの指示インターフェース実装（練習問題生成など）           |        |      | 完了   |  練習問題・クイズ生成機能（AI指示）実装の方で進捗を管理するため、こちらは完了とする                                        |
| 設定画面実装（必要最低限）                                   | AI/USER |      | 進行中   | ヘッダーの「設定」ドロップダウンメニュー項目を「フィードバックを送信」「利用規約」に整理。フィードバック送信機能（モーダルUI、Resend経由でのメール送信API）を実装完了。 |
...
| 参照ソース明示機能実装                                 | AI/USER |      | 完了   | 回答とマニュアルの該当箇所を紐付け。AIが自動的にページ番号と参照元を表示する機能として実装完了。チャット時の参照ソース選択機能と統合。         |
| 介護特化チューニング（専門用語辞書組込）               |        |      | 対象外   | 今回の開発スコープからは除外。                           |
| 回答品質と速度の継続的な監視・調整                     | AI/USER | -    | 完了     | Langsmith等を用いた評価とパラメータ最適化。メタプロンプティング導入により精度向上したため一旦完了。 |
...
| 練習問題・クイズ生成機能（AI指示）実装                               | AI/USER |      | 完了     | 結果を「管理者作成メモ」に保存、採点手動                     |
| OCR機能実装（テキスト抽出困難なPDFへの対応）                        | AI/USER |      | 完了     | Google Cloud Vision API統合によるOCR機能実装完了。テキスト抽出が不十分なPDF（100文字未満または50文字/ページ未満）に対して自動的にOCR処理を実行。抽出テキストは「[OCR抽出テキスト]」マーカーで識別可能。HTTPリクエスト方式採用によりEdge Function軽量化。ファイルサイズ制限（20MB）、エラーハンドリング、サニタイズ処理を含む。 |
| AIによる次アクション提案機能（レコメンド機能）                       | AI/USER |      | 完了   | **セクション名変更:** 「資料の活用アイデア」に変更済み。<br>**仕様詳細:** <br>- 更新トリガー: 「アイディアを更新」ボタン押下時のみ。 <br>- 初回表示: 「『アイディアを更新』ボタンを押して、最初の提案を取得しましょう」等の案内。 <br>- 更新中表示: ローディングスピナー等の表示。 <br>- 生成不可時: 「適切な提案が見つかりませんでした」等のメッセージ。 <br>- 最大表示数: 10件（5行2列イメージ）。 <br>- キャッシュ: localStorage等に保存しリロード後も保持。有効期限2日。キャッシュ構造検証済み。 <br>- エラー処理: APIエラー時に適切なメッセージ表示。 <br>- 対象ソース: 初期は全ソース、将来的にはユーザー選択ソース（現在選択ソースをAPIに送信）。<br>- UI: 提案カードのパディング調整、タイトルフォントサイズ調整、説明文のツールチップ表示（デフォルトスタイルに戻し）、登場時アニメーション（ぽこぽこ表示）実装済み。 |
| PDF出力機能実装                                                  |        |      | 対象外   | DBに格納しているHTMLファイルと、出力システムの互換性が悪く、ユーザーがメモ帳から直接コピー＆ペーストする方がユーザビリティが高いため。 |
| Word出力機能実装                                                 |        |      | 対象外   | DBに格納しているHTMLファイルと、出力システムの互換性が悪く、ユーザーがメモ帳から直接コピー＆ペーストする方がユーザビリティが高いため。 |

## 6. アクセス管理機能開発
| タスク名                                                     | 担当者 | 期限 | 進捗状況 | 備考                                 |
| :----------------------------------------------------------- | :----- | :--- | :------- | :----------------------------------- |
| ユーザー認証システム実装 (Clerk)                             |        |      | 完了     | 管理者向けログイン (注: 初期調査含め後続フェーズで対応) |
| Clerkリダイレクト設定修正（複数サービス対応）                | USER   |      | 進行中   | ClerkProviderにafterSignInUrl等を追加済み。USER側でプッシュ・デプロイ後、Vercelカスタムドメイン設定とClerk Dashboard設定更新予定。 |
| ログイン不要オプション検討・実装（従業員向け）               | AI/USER |      | 完了   | 共有URL機能として実装完了。ユーザーごとの共有ID生成、ソース選択状態同期、認証不要アクセス、永続的有効期限（30日制限削除）、Edge Function経由でのRLSバイパス対応。Hydrationエラー修正、共有モーダルUI強調表示対応完了。 |
| Clerk/Supabase RLS連携ベストプラクティス検討(service_role_key利用等) | USER/AI |  | 完了   | Edge FunctionでのDBアクセスにおけるセキュリティとRLSの適切な連携方法を再検討 |
| フリーミアム課金機能（Stripe決済連携）                       | USER/AI |      | 未着手   | Stripeを用いた有料プラン決済・サブスクリプション管理。無料枠・有料枠の切り替え、決済UI、利用制限ロジックの設計・実装。 |

## 7. 多言語対応機能開発
| タスク名                         | 担当者 | 期限 | 進捗状況 | 備考          |
| :------------------------------- | :----- | :--- | :------- | :------------ |
| AIチャットボットの多言語応答対応 | AI/USER |      | 完了     | Google Geminiの基本機能でベトナム語応答を確認 |
